...
The local VLM and token-ID scenarios (`realtime_video_understanding`, `chat_token`) are not phases of the full suite. Run them with `run_vlm_test.sh` / `run_chat_test.sh`, or directly through the orchestrator (see [Running VLM & Token ID Scenarios](#running-vlm--token-id-scenarios)).
